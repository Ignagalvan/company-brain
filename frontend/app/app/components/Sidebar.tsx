...
export function Sidebar({
  ...
  async function handleUpload() {
    ...
    try {
      ...
      setUploadMessage(`"${clean}" procesado.`)
      onDocUploaded({ id: Date.now(), filename: clean })
      clearFile()
    } catch {
    ...
    <aside className="w-72 shrink-0 bg-white border-r border-slate-100 flex flex-col overflow-hidden">
        ...
        {conversations.length === 0 ? (
          <div className="px-4 py-5">
            <p className="text-[11px] text-slate-400 leading-relaxed">
              No hay conversaciones todavía. Iniciá una nueva para comenzar.
            </p>
          </div>
        ) : (
          <ul className="py-2">
            {conversations.map(conv => {
              ...
              return (
                <li key={conv.id} className="group flex items-stretch border-l-2 transition-colors
                ...
                >
                  <button
                    ...
                    className={[
                      'flex-1 min-w-0 text-left px-4 py-2.5 flex items-start gap-2.5 transition-colors',
                  ...
                  <button
                    ...
                    className={[
                      'shrink-0 px-2 text-[13px] transition-all rounded-sm',
      ...
      <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-4 space-y-4">
        ...
        <div>
          ...
          {uploadedDocs.length === 0 ? (
            <p className="text-[11px] text-slate-400">Ningún documento cargado aún.</p>
          ) : (
            <ul className="space-y-1">
              {uploadedDocs.map(doc => (
                <li key={doc.id} className="flex items-center gap-2 px-2.5 py-1.5 bg-white rounded-lg border border-slate-100">
