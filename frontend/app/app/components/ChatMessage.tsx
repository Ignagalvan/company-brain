...
function EvidenceBadge({ msg }: { msg: Message }) {
  if (msg.has_sufficient_evidence) {
    return (
      <div className="space-y-1.5">
        <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-100 px-3 py-1.5 rounded-full">
          <span>✓</span> Evidencia suficiente
        </div>
        <p className="text-xs text-slate-500">
          Basado en {msg.sources_count} fragmento{msg.sources_count !== 1 ? 's' : ''} de {msg.documents_count} documento{msg.documents_count !== 1 ? 's' : ''}
        </p>
      </div>
    )
  }
  ...
  if (msg.is_partial_answer) {
    return (
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-slate-500">
          Respuesta parcial — {msg.sources_count} fragmento{msg.sources_count !== 1 ? 's' : ''} de evidencia encontrado{msg.sources_count !== 1 ? 's' : ''}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-1.5">
      <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-full">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <span className="shrink-0 text-xs text-slate-400 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-md">
              fragmento {citation.chunk_index}
            </span>
          </div>
          <button
            onClick={onClose}
            className="shrink-0 ml-3 text-slate-400 hover:text-slate-600 text-xl leading-none"
            aria-label="Cerrar"
          >
            ×
          </button>
        ...
        <div className="overflow-y-auto px-6 py-5">
          ...
          <div className="text-sm text-slate-700 leading-7 whitespace-pre-wrap">
            {highlight ? (
              ...
              <span className="bg-amber-50 block rounded-lg px-4 py-3 border border-amber-100">
                {citation.content}
              </span>
            )}
          </div>
      ...
      <div className="max-w-2xl w-full space-y-2.5">
        ...
        <div className="animate-fade-slide-in bg-white border border-slate-200 rounded-2xl rounded-tl-sm shadow-sm">
          <div className="flex items-center justify-between px-6 pt-4 pb-0">
            ...
            <button
              onClick={handleCopy}
              className="text-xs text-slate-400 hover:text-slate-600 px-2.5 py-1 rounded-md hover:bg-slate-100 transition-colors"
            >
              {copied ? '✓ Copiado' : 'Copiar'}
            </button>
          </div>
          <div className="px-6 py-5">
            <p className="text-[15px] text-slate-800 leading-7">{msg.content}</p>
          </div>
        ...
        {hasCitations && (
          <div className="pl-1">
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-slate-600 transition-colors px-1 py-1"
            >
                ...
                {msg.citations.map((citation, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 px-4 py-3 bg-slate-50 border-b border-slate-100">
                      ...
                      <span className="shrink-0 text-xs text-slate-400 bg-white border border-slate-200 px-2 py-0.5 rounded-md">
                        fragmento {citation.chunk_index}
                      </span>
        ...
        {showDebug && msg.debug && (
          <div className="mt-1 p-4 bg-slate-900 rounded-xl font-mono text-xs text-slate-300 space-y-2">
            <p className="text-slate-400 font-semibold uppercase tracking-widest text-[10px]">Debug</p>
            <p>
            ...
            <div>
              <p className="text-slate-500 mb-1">distances:</p>
